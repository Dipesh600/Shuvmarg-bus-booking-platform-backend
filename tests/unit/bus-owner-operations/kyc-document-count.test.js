...
test("kyc-document-count unit tests", async (t) => {
  ...
  await t.test("single section document URL counts 1 each", () => {
    ...
    assert.equal(countBusOwnerKycDocuments({ ownerIdentity: { documentUrls: ["identity.pdf"] } }), 1);
  });

  await t.test("counts only the three business KYC documents", () => {
    const owner = {
    ...
    assert.equal(countBusOwnerKycDocuments(owner), 3);
  });
});
