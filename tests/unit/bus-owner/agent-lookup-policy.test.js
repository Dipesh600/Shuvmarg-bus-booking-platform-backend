...
const policy = require('../../../src/modules/bus-owner/agent-lookup/bus-owner-agent-lookup.policy');
...
test('lookupFilterFor', async (t) => {
  await t.test('accepts a canonical code', () => {
    assert.deepEqual(policy.lookupFilterFor(VALID_CODE), { code: VALID_CODE });
  });

  await t.test('canonicalises what a human pastes', () => {
    assert.deepEqual(policy.lookupFilterFor(`  ${VALID_CODE.toLowerCase()}  `), { code: VALID_CODE });
  });

  await t.test('accepts the legacy form on its own field', () => {
    assert.deepEqual(policy.lookupFilterFor(LEGACY_ID), { agentId: LEGACY_ID });
  });
  ...
  await t.test('returns null for anything that cannot be a code', () => {
    ...
    assert.equal(policy.lookupFilterFor('SM-AG-MAVSKNG'), null, 'a bad check symbol is not a code');
    assert.equal(policy.lookupFilterFor('SM-BR-MAVSKNF'), null, 'a brand code is not an agent code');
    assert.equal(policy.lookupFilterFor('nonsense'), null);
    assert.equal(policy.lookupFilterFor(''), null);
    assert.equal(policy.lookupFilterFor(undefined), null);
    assert.equal(policy.lookupFilterFor(null), null);
    assert.equal(policy.lookupFilterFor(12345), null);
  });

  await t.test('refuses input too long to be a typo', () => {
    assert.equal(policy.lookupFilterFor('A'.repeat(policy.MAX_CODE_INPUT_LENGTH + 1)), null);
    // A regex-flavoured payload never reaches the query builder.
    assert.equal(policy.lookupFilterFor(`${VALID_CODE}${'|'.repeat(64)}`), null);
  });

  await t.test('rejects an object, so no operator filter can be smuggled in', () => {
    assert.equal(policy.lookupFilterFor({ $ne: null }), null);
    assert.equal(policy.lookupFilterFor(['SM-AG-MAVSKNF']), null);
  });
});
...
test('isAssignableByOperator', async (t) => {
  ...
  await t.test('classifies a legacy row by its agentType', () => {
    // The reason the projection carries `agentType`: without it these rows read
