...
describe("Stop Bulk Import Service", () => {
  ...
  describe("Real insertMany() Bulk Write Failure Coverage", () => {
    it("forces insertMany write failure, preserving original row mapping and partial insert counts", async () => {
      const payload = [
        { code: "INVALID0", name: "" },           // Row 0: Filtered out before insertMany
        { code: "VALID1", name: "Valid Stop One" },// Row 1: Valid doc -> Batch index 0 (inserted)
        { code: "VALID2", name: "Valid Stop Two" },// Row 2: Valid doc -> Batch index 1 (write error)
      ];
      ...
      try {
        Stop.insertMany = async () => {
          ...
          err.writeErrors = [
            {
              index: 1, // Batch index 1 corresponds to original Row 2 (VALID2)
              code: 11000,
              keyPattern: { code: 1 },
        ...
        assert.equal(result.inserted, 1, "Partial count: 1 inserted doc");
        assert.equal(result.skipped, 2, "Partial count: 2 skipped docs");
        assert.equal(result.invalidCount, 2, "2 total error items");

        const row0Error = result.errors.find((e) => e.index === 0);
        const row2Error = result.errors.find((e) => e.index === 2);

        assert.ok(row0Error, "Row 0 error exists");
        assert.equal(row0Error.errorCode, "INVALID_STOP_DATA");

        assert.ok(row2Error, "Row 2 error exists (mapped from batch index 1)");
        assert.equal(row2Error.code, "VALID2");
        assert.equal(row2Error.errorCode, "STOP_CODE_CONFLICT");
        assert.equal(row2Error.message, "A stop with this code already exists.");

        const row1Error = result.errors.find((e) => e.index === 1);
        assert.equal(row1Error, undefined, "Row 1 successfully inserted, no error reported");
      } finally {
          ...
          err.writeErrors = [
            {
              index: 0,
              code: 121,
              errmsg: "Document failed validation in MongoDB",
            },
          ];
          err.insertedDocs = [];
          throw err;
          ...
          err.writeErrors = [
            {
              index: 0,
              code: 11000,
              keyPattern: { _normalizedIdentity: 1 },
    ...
    it("logs internal bulk write failure using logger.error", async () => {
      ...
      try {
        ...
        assert.ok(loggedMetadata, "logger.error was called");
        assert.equal(loggedMetadata.mongoCode, 11000);
        assert.equal(loggedMetadata.writeIndex, 0);
        assert.equal(loggedMetadata.sourceIndex, 0);
        assert.equal(loggedMetadata.stopCode, "LOGSTOP");
        assert.equal(loggedMetadata.stopName, "Logging Stop");
      } finally {
