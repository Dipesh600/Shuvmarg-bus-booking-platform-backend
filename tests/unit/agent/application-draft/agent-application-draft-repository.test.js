...
test('agent application draft repository', async (t) => {
    await t.test('findAgentByUserId calls Agent.findOne with { user: userId }', async () => {
        ...
        Agent.findOne = (filter) => {
            calls.push(filter);
            return Promise.resolve({ agentId: 'AG-FOUND' });
        };
        try {
    ...
    await t.test('createAgentForUserId returns new Agent({ user: userId })', async () => {
        const userId = crypto.randomBytes(12).toString('hex');
        const agent = repository.createAgentForUserId(userId);
        // Verify it is a Mongoose document instance with user set
        assert.ok(agent);
        assert.ok(typeof agent.save === 'function', 'should be a Mongoose document with .save()');
        // The user field should match the provided userId
        assert.equal(agent.user.toString(), userId.toString());
    });

    await t.test('saveAgent calls agent.save()', async () => {
        const calls = [];
