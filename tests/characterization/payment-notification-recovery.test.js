...
let data;
before(async () => { await fixture.start(); await Promise.all([Job, Cashback, Notification, Devices].map(model => model.init())); });
after(fixture.stop);
beforeEach(async () => {
  data = await fixture.seed();
  await Promise.all([Job, Cashback, Notification, Devices].map(model => model.deleteMany({})));
...
test("failed booking commit rolls back all follow-up records", async () => {
  await Promise.all([Job, Cashback, Notification].map(model => model.deleteMany({})));
  ...
  for (const model of [Job, Cashback, Notification]) assert.equal(await model.countDocuments({}), 0);
});
