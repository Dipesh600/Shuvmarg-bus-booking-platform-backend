...
const confirmationRepository = require('../../src/modules/booking/passenger-seat-hold/passenger-seat-hold-confirmation.repository');
...
test('Passenger Seat Hold — Database Concurrency & Single-Field Unique Indexes', async (t) => {
  ...
  await t.test('parallel competing holds for same seat: exactly one succeeds, one receives 409', async () => {
    const results = await Promise.allSettled([
      service.createOrReusePassengerSeatHold({ userId: user1, tripId, seatNumbers: ['a1'], originalAmount: 100 }),
      service.createOrReusePassengerSeatHold({ userId: user2, tripId, seatNumbers: ['a1'], originalAmount: 100 }),
    ]);
  ...
  await t.test('parallel requests for disjoint seats both succeed', async () => {
    const [h1, h2] = await Promise.all([
      service.createOrReusePassengerSeatHold({ userId: user1, tripId, seatNumbers: ['a1'], originalAmount: 100 }),
      service.createOrReusePassengerSeatHold({ userId: user2, tripId, seatNumbers: ['a2'], originalAmount: 100 }),
    ]);
  ...
  await t.test('expired overlapping seat hold does not block another passenger', async () => {
    ...
    await service.createOrReusePassengerSeatHold({ userId: user1, tripId, seatNumbers: ['a1'], originalAmount: 100, now: past });
    const h2 = await service.createOrReusePassengerSeatHold({ userId: user2, tripId, seatNumbers: ['a1'], originalAmount: 100, now: new Date() });
    assert.ok(h2.tempBookingId);
  });

  await t.test('parallel confirmation claims allow exactly one payment flow', async () => {
    const hold = await service.createOrReusePassengerSeatHold({
      userId: user1, tripId, seatNumbers: ['a1'], originalAmount: 100,
    });
    const [first, second] = await Promise.all([
      confirmationRepository.claimOwnedActiveHold(
        hold._id, user1, hold.expiresAt
      ),
      confirmationRepository.claimOwnedActiveHold(
        hold._id, user1, hold.expiresAt
      ),
