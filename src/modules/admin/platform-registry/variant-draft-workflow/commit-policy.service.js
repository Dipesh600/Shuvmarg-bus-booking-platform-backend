...
function assertCandidatesReady(candidates, variant) {
  ...
  if (variant.originTerminalStopId && (!included[0].isTerminal || !sameId(included[0].resolvedStopId, variant.originTerminalStopId))) {
    throw routeVariantError("VARIANT_TERMINAL_SEQUENCE_MISMATCH", "The reviewed route must begin at the selected physical origin terminal.");
  }
  if (variant.destinationTerminalStopId && (!included.at(-1).isTerminal || !sameId(included.at(-1).resolvedStopId, variant.destinationTerminalStopId))) {
    throw routeVariantError("VARIANT_TERMINAL_SEQUENCE_MISMATCH", "The reviewed route must end at the selected physical destination terminal.");
  }
  return included;
}
