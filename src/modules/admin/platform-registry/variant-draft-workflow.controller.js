...
function sendError(res, error) { if (!error?.code || !Number.isInteger(error.statusCode)) { console.error("[variant-draft-workflow] unexpected error", error);
...
function actorId(req) { return req.adminInfo?.id || null;
}
async function createVariantDraft(req, res) { try { const data = await workflow.createVariantDraft(
...
async function previewCorridorRoutePaths(req, res) { try { const data = await workflow.previewCorridorRoutePaths(
...
async function getVariantDraft(req, res) { try { const data = await workflow.getVariantDraft(req.params.variantId, { includeRouteGeometry: req.query.includeRouteGeometry === "true",
    });
    return res.status(200).json({ success: true, data });
  } catch (error) { return sendError(res, error);
  }
}
async function refreshVariantDraftRouteOptions(req, res) { try { const data = await workflow.refreshVariantDraftRouteOptions(
...
async function searchVariantDraftGuidancePlaces(req, res) { try { const data = await workflow.searchVariantDraftGuidancePlaces(
...
async function selectVariantDraftRouteOption(req, res) { try { const data = await workflow.selectVariantDraftRouteOption(
...
async function updateVariantDraftDetails(req, res) { try { const data = await workflow.updateVariantDraftDetails(
...
async function prepareVariantDraftStopCandidates(req, res) { try { const data = await workflow.prepareVariantDraftStopCandidates(req.params.variantId);
    return res.status(200).json({ success: true, message: "Stop candidates prepared for review.", data });
  } catch (error) { return sendError(res, error);
  }
}
async function updateVariantDraftCandidate(req, res) { try { const data = await workflow.updateVariantDraftCandidate(
...
async function useAllMatchedVariantDraftCandidates(req, res) { try { const data = await workflow.useAllMatchedVariantDraftCandidates(req.params.variantId);
    return res.status(200).json({ success: true, message: "All safe existing Stop matches are selected.", data });
  } catch (error) { return sendError(res, error);
  }
}
async function addExistingVariantDraftStop(req, res) { try { const data = await workflow.addExistingStopCandidate(req.params.variantId, req.body.stopId);
    return res.status(200).json({ success: true, message: "Stop added and positioned on the selected road path.", data });
  } catch (error) { return sendError(res, error);
  }
}
async function commitVariantDraft(req, res) { try { const data = await workflow.commitVariantDraft(req.params.variantId, actorId(req));
...
module.exports = { addExistingVariantDraftStop,
