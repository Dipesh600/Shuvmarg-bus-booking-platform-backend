...
function handleError(res, error) {
  ...
  if (error.code === 11000) {
    return res.status(409).json({
      success: false, message: "A stop with this identity already exists.", code: "DUPLICATE_ERROR",
    });
  }
  if (error.message && error.message.startsWith("REFERENCED:")) {
    const parts = error.message.split(":");
    return res.status(409).json({
      success: false, message: parts.slice(2).join(":"), refCount: parseInt(parts[1], 10),
    });
  }
...
async function createStop(req, res) {
  try {
    ...
    res.status(201).json({ success: true, message: "Stop added to registry.", data });
  } catch (error) {
...
async function getAllStops(req, res) {
  try {
    const filter = {};
    const {
      isSearchable, isRouteStop, parentStopId, status,
      verificationStatus, source, type, district, municipality,
    } = req.query;
    ...
    if (parentStopId !== undefined) filter.parentStopId = parentStopId === "null" ? null : parentStopId;
...
async function previewBulkImportStops(req, res) {
  try {
    ...
    const invalid = ["too large", "empty", "array"].some((term) => error.message.includes(term));
    res.status(invalid ? 400 : 500).json({ success: false, message: error.message });
  }
}
...
async function bulkImportStops(req, res) {
  try {
    ...
    res.status(200).json({
      success: true, message: `Import complete. ${data.inserted} stop(s) added, ${data.skipped} skipped.`, data,
    });
  } catch (error) {
    const invalid = ["too large", "empty", "array"].some((term) => error.message.includes(term));
    res.status(invalid ? 400 : 500).json({ success: false, message: error.message });
