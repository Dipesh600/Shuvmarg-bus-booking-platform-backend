...
const repository = require('./agent-login.repository');
const policy = require('./agent-login.policy');
const errors = require('./agent-login.errors');
...
const assertAccountAllowed = (user) => {
  ...
  if (!policy.resolveRoles(user).includes('agent')) {
    throw errors.missingAgentRoleError();
  }
};
...
const forcePasswordResult = (user) => {
  const tempToken = jwt.sign(
    { id: user._id, purpose: 'FORCE_PASSWORD_CHANGE', activeRole: 'agent' },
