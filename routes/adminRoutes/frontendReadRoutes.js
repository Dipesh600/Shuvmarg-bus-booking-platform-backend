...
function registerAdminFrontendReadRoutes(router, options = {}) {
  ...
  router.post("/getBusOwnerDetails", auth, ownerCtrl.getBusOwnerById);
  ...
  router.post("/getBusOwnerKycDetails", auth, ownerCtrl.getBusOwnerKycById);
  ...
  router.get("/fleet/:id/setup-status", auth, fleetCtrl.getFleetSetupStatus);
  router.get("/fleet/getById/:id", auth, fleetCtrl.getFleetById);

  // LEGACY COMPATIBILITY ROUTE.
  // Do not use for new frontend integrations.
  // Canonical fleet detail:
  // GET /api/admin/fleets/:fleetId
  router.get("/fleet/details/:id", auth, adminFleetController.getFleetById);
