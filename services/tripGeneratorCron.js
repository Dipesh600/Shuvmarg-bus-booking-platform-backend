...
const { resolveForSchedule } = require("./seatLayoutSnapshotService.js");
const {
    projectLegacySeatArrays,
} = require("../src/domain/seat-layout/seat-layout.legacy-projection.js");
const {
    assertScheduleRouteChainReady,
} = require("../src/modules/admin/schedule-management/schedule-route-chain.policy.js");
const seatLayoutRevisions = require(
    "../src/modules/fleet-management/seat-layout-revision.js"
);
 ...
 */
const initializeSeats = async (tripId, seatConfig) => {
    const { seata, seatb, seatc } = projectLegacySeatArrays(seatConfig);
...
const setupTripGeneratorCron = () => {
    ...
    cron.schedule("1 0 * * *", async () => {
        ...
        try {
            const now = new Date();
            await seatLayoutRevisions.applyDueRevisions(now);
...
const createTripFromSchedule = async (schedule, tripDateStart, dateStr) => {
    ...
    const layout = await resolveForSchedule(schedule, tripDateStart);
    ...
    try {
        await initializeSeats(newTrip._id, layout.seatLayoutSnapshot.seatConfig);
    } catch (error) {
        await Trip.findByIdAndDelete(newTrip._id).catch(() => {});
        throw error;
    }
