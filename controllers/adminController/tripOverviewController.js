...
const getScheduleHealth = async (req, res) => {
    try {
        const { shouldGenerate, isAlternatingDay } = require("../../services/tripGeneratorCron.js");
        ...
        // For each schedule, compute comprehensive health data
        const healthData = [];
        const warnings   = [];
        ...
        for (const sched of schedules) {
            ...
            // First ever trip for this schedule
            ...
            // Last successfully generated trip (most recent by creation)
            const lastGeneratedTrip = await Trip.findOne({
                scheduleId: sched._id,
                isAutoGenerated: true,
            })
                .sort({ createdAt: -1 })
                .select("tripDate createdAt tripId")
                .lean();
            ...
            // ── Compute exact missing dates ──────────────────────────────────
            // Walk from today through the expected horizon and identify dates
            // that should have a trip but don't.
            const missingDates = [];
            const scanStart = new Date(now);
            scanStart.setUTCHours(0, 0, 0, 0);
            // Only scan if schedule is ACTIVE (suspended ones are expected to have gaps)
            if (sched.status === "ACTIVE" && gapDays > 0) {
                // Fetch all existing trip dates for this schedule in the scan window
                const existingTrips = await Trip.find({
                    scheduleId: sched._id,
                    tripDate: { $gte: scanStart, $lte: expectedDate },
                    status: { $ne: "cancelled" },
                })
                    .select("tripDate")
                    .lean();
                ...
                const existingDateSet = new Set(
                    existingTrips.map(t => new Date(t.tripDate).toISOString().split("T")[0])
                );

                // Walk each day from tomorrow through expected horizon
                const scanFrom = new Date(scanStart);
                scanFrom.setUTCDate(scanFrom.getUTCDate() + 1); // start from tomorrow

                for (let d = new Date(scanFrom); d <= expectedDate; d.setUTCDate(d.getUTCDate() + 1)) {
                    const dateStr = d.toISOString().split("T")[0];
                    const dayOfWeek = d.getUTCDay();

                    // Check effective period
                    if (d < new Date(sched.effectiveFrom)) continue;
                    if (sched.effectiveUntil && d > new Date(sched.effectiveUntil)) continue;

                    // Check recurrence
                    if (!shouldGenerate(sched, dayOfWeek)) continue;

                    // Check alternating day (RELAY)
                    if (!isAlternatingDay(sched, d)) continue;

                    // If no trip exists for this date, it's missing
            ...
            // ── Suspension-specific enrichment ───────────────────────────────
            let suspensionInfo = null;
            if (sched.status === "SUSPENDED") {
                const suspendedSince = sched.suspendedAt || sched.updatedAt;
                const daysSuspended = Math.round(
                    (now.getTime() - new Date(suspendedSince).getTime()) / 86400000
                );

                // Count how many trips would have been generated during the suspension
                let missedDuringSuspension = 0;
                if (suspendedSince) {
                    const suspStart = new Date(suspendedSince);
                    suspStart.setUTCHours(0, 0, 0, 0);
                    for (let d = new Date(suspStart); d <= now; d.setUTCDate(d.getUTCDate() + 1)) {
                        const dayOfWeek = d.getUTCDay();
                        if (d < new Date(sched.effectiveFrom)) continue;
                        if (sched.effectiveUntil && d > new Date(sched.effectiveUntil)) continue;
                        if (!shouldGenerate(sched, dayOfWeek)) continue;
                        if (!isAlternatingDay(sched, d)) continue;
                        missedDuringSuspension++;
                    }
                }

                suspensionInfo = {
                    suspendedAt: suspendedSince,
                    daysSuspended,
                    missedTrips: missedDuringSuspension,
                    autoResumeDate: sched.suspendUntil || null,
                    reason: sched.suspensionReason || null,
                };
            }
            ...
            const entry = {
                ...
                health: {
                    ...
                    lastGeneratedDate: lastGeneratedTrip?.tripDate || null,
                    lastGeneratedAt: lastGeneratedTrip?.createdAt || null,
                    firstTripDate: firstTrip?.tripDate || null,
                    windowDays,
                },
                suspensionInfo,
            };
        ...
        // Summary KPIs
        const totalMissing = healthData.reduce((s, h) => s + h.health.missingCount, 0);
        const kpis = {
