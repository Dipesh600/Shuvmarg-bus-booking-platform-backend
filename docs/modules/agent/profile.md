...
The Agent profile module returns the approved agent’s operational profile, linked operator reference, settlement fields, commission counters and marketing identifiers.
...
- Preserve the handler’s defensive missing-profile and approval checks.
...
4. `requireApprovedAgent` requires an approved Agent application.
...
- Non-approved Agent returns `403` with the dynamic message `Your application is "<status>". Profile is available after approval.` and `data.applicationStatus`.
...
5. If `applicationStatus` is not `APPROVED`, return the defensive dynamic `403`.
...
The operation relies on a current access token, DB-backed User role verification, active role `agent`, approved Agent application status, and the Agent document whose `user` field matches the authenticated user ID.
...
| `Agent` | `requireApprovedAgent` reads approval status. The module reads the Agent by `user` and populates `linkedOperatorId`. | None. |
...
- Keep the handler’s defensive approval check even though `requireApprovedAgent` runs first.
...
- `tests/characterization/agent-profile.test.js` verifies the public route, middleware chain, approval protection, linked-operator response, null linked operator response and exact success fields.
...
The route-level `requireApprovedAgent` middleware usually rejects missing or non-approved Agent records before the profile module runs. The profile module still preserves its own `404` and dynamic `403` defensive responses.
