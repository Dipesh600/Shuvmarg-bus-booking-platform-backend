...
log "  2. Set GHCR_TOKEN in GitHub repository secrets (read-only package token)"
log "  3. Set ORACLE_STAGING_HOST, ORACLE_STAGING_USER, ORACLE_STAGING_SSH_KEY,"
log "     ORACLE_STAGING_SSH_PORT in GitHub repository secrets"
log "  4. Ensure Oracle Cloud Security List allows TCP 22, 80, 443 (NOT 7012)"
log "  5. Point ${STAGING_API_DOMAIN:-api-staging.shuvmarg.com} DNS A record to this VM's public IP"
log "  6. Push to the staging branch to trigger the first deployment"
